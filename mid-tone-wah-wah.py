# deep_swell_blob_pitch_volume_with_lights.py
...
blob_size = 30  # or 10, or whatever size you want
num_of_lights = 336
...
def make_triangle_wave(samples=256):
    half = samples // 2
    up = list(range(-30000, 30000, int(60000 / half)))
    down = list(range(30000, -30000, -int(60000 / half)))
    return array.array("h", up + down[:len(up)])

waveform = make_triangle_wave()
...
# --- Envelope for slow, massive swell ---
amp_env = synthio.Envelope(attack_time=0.6, release_time=1.2)
...
print("🌊 Deep Swell Blob (Pitch + Volume + Light) running!")
...
def update_lights(norm, amp, fade=False):
    """Update the strip based on normalized distance (0.0-1.0) and amplitude (0.0-1.0)."""
